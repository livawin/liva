# Liva (LIVA)
Decentralized Prediction Markets on Solana

Liva (LIVA) is a utility and governance token powering the Liva decentralized prediction market ecosystem on Solana. It enables users to predict outcomes, stake tokens, and earn rewards across sports, crypto markets, and real-world events.

---

## Official Links

- Website: https://liva.win
- GitHub: https://github.com/livawin/liva
- Telegram: https://t.me/liva_win
- X (Twitter): https://x.com/livawin

---

## What Liva Is

Liva is designed to make prediction markets transparent and accessible by using on-chain settlement primitives and publicly verifiable token distribution.

LIVA is the platform’s core token for:

- Prediction market participation
- Rewards and incentives
- Governance
- Liquidity support and price discovery

---

## Token Overview

| Property | Value |
|--------|------|
| Token Name | Liva |
| Symbol | LIVA |
| Blockchain | Solana |
| Token Standard | SPL |
| Decimals | 6 |
| Total Supply | 10,000,000,000 LIVA |
| Mint Authority | Revoked |
| Freeze Authority | Disabled |
| Vesting Provider | Streamflow |
| Primary Use | Prediction markets, staking, governance |

---

## Token Utility

### Prediction Markets
- Stake LIVA on sports, crypto price movements, and real-world events
- Outcomes resolved using oracle data
- Rewards distributed proportionally to correct predictions

### Staking and Incentives
- Long-term staking to support protocol stability
- Reward emissions aligned with platform usage

### Governance
- LIVA holders participate in governance decisions
- Influence market parameters and protocol upgrades

### Liquidity
- DEX liquidity provisioning
- Improved market depth and price discovery

---

## Token Distribution and Vesting

All vesting allocations are enforced on-chain via Streamflow.  
All vested tokens unlock linearly on a monthly basis.

### Allocation Table

| Allocation | % | Exact Amount (LIVA) | Wallet | Vesting |
|----------|---|--------------------|--------|---------|
| Initial Liquidity | 10% | 1,000,000,000 | 61cPxiqGBrdV2eQLvqAWe7GpaTWss4QJhJNN8h5xEFKN | Instant |
| Liquidity Reserve | 10% | 1,000,000,000 | EXPcmZBzNSZHUsvef3HRz5R5mEAgRWPB4ZXYtd4mZXLc | 12 Months |
| Liquidity Management | 5% | 500,000,000 | 61cPxiqGBrdV2eQLvqAWe7GpaTWss4QJhJNN8h5xEFKN | Instant |
| Head Manage | 1% | 100,000,000 | JCQdZvpcBLuXf4xGRWWnrUzLCduukaAzUe2ggnyXxnZu | 24 Months |
| Advertisement | 10% | 1,000,000,000 | E1ZMEtAfu4VFh8V9JCssf7f9pDVUTLmQiJoFg3EwUH89 | 12 Months |
| Team Wallet 1 | 20% | 2,000,000,000 | CPRvYwXzfvDeXs7PB17QNJSvkzQvtDkaez1ZHNfaXPgS | 24 Months |
| Team Wallet 2 | 20% | 2,000,000,000 | D8n2MUC1G92a28jfdTi8ZeDaKLpimnx6WiHSxta6x8Jp | 24 Months |
| Team Wallet 3 | 10% | 1,000,000,000 | 2vHH8KurVmYPLeDuebsrfGpw67FVG4g3QBvawThN4dwF | 24 Months |
| Development | 7% | 700,000,000 | EPD9brHxmerCAgMVVDfCrxz7aeNn4M2etAfZXNcipd9g | 12 Months |
| Service Provider | 5% | 500,000,000 | 8WAERDnWei1d9iHjD5os81jnHgKuyqKKDJP5CbY4Y2Wn | 12 Months |
| Staking Rewards | 2% | 200,000,000 | 61cPxiqGBrdV2eQLvqAWe7GpaTWss4QJhJNN8h5xEFKN | 12 Months |

Total Supply: 10,000,000,000 LIVA (100%)

---

## Vesting Guarantees

- All vested allocations locked via Streamflow
- No manual unlocks or overrides
- Linear monthly unlock schedules
- Publicly verifiable on-chain
- Designed to reduce early sell pressure

---

## Security and Transparency

### Token Security
- Standard SPL token
- Mint authority revoked
- Freeze authority disabled
- No blacklist or transfer restrictions

### Transparency
- No hidden allocations
- No inflation beyond declared supply
- Public wallets disclosed

---

## Disclaimer

LIVA is a utility token for platform usage and governance.  
Prediction markets involve risk. Users should conduct their own research before participating.
